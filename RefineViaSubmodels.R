...
#   sName: name of "S", the categorical var. (only 1 allowed), 
#      an R factor
#   qeFtnName: name of the qe-series ML predictive function; 
#      currently, only default args can be used
#   nReps: number of holdout/cross-validation runs

RefineViaSubmodels <- function(data,yName,sName,qeFtnName) 
{
   ...
   overallWithS <- function() 
   {
      cmd <- sprintf("%s(data,yName,sName)testAcc",qeFtnName)
      runCmd(cmd)
   }

   overallWithNoS <- function() 
   {
      thisData <- data[,sCol]
      cmd <- sprintf("%s(thisData,yName,sName)$testAcc",qeFtnName)
      runCmd(cmd)
   }

   withinSclass <- function(sLevel) 
   {
      thisData <- data[rowNumsEachClass[[sLevel]],-sCol]
      cmd <- sprintf("%s(thisData,yName,sName)$testAcc",qeFtnName)
      runCmd(cmd)
   }

   specificSclassFullDataWithS <- function(sLevel) 
   {
      xToPredict <- data[rowNumsEachClass[[sLevel]],-c(ycol,sCol)]
      preds <- predict(fullFitWithS,xToPredict)
   }
